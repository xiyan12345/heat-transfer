...
100          FORMAT("ZONE I = ", I3, ",J = ", I3, ",K = ", I3, ",DATAPACKING = POINT")
        ...
        subroutine save_Tecplot_Debug(fileName)
            implicit none
           ...
           ! 中间变量
            ...
            LAPs = 1
            open(unit = un_result, file = fileName, status = "REPLACE")
            write(un_result,"(A39)",advance="NO") "variables = ""I"",""J"",""K"",""x"",""y"",""z"",""T"""
            zone_format = "('zone I=',I??,',J=',I??,',K=',I??,',DATAPACKING=BLOCK,VARLOCATION=([7]=CELLCENTERED)')"
            do m = 1, num_Block
                ...
                do k = 1-LAPs, B%nz+LAPs
                    do j = 1-LAPs, B%ny+LAPs
                        do i = 1-LAPs, B%nx+LAPs
                            if (mod(count,3)==0) write(un_result,*)
                            count = count+1
                            write(un_result,"(G24.15)",advance="NO") I 
                        end do
                    end do
                end do
                ...
                do k = 1-LAPs, B%nz+LAPs
                    do j = 1-LAPs, B%ny+LAPs
                        do i = 1-LAPs, B%nx+LAPs
                            if (mod(count,3)==0) write(un_result,*)
                            count = count+1
                            write(un_result,"(G24.15)",advance="NO") j 
                        end do
                    end do
                end do
                ...
                do k = 1-LAPs, B%nz+LAPs
                    do j = 1-LAPs, B%ny+LAPs
                        do i = 1-LAPs, B%nx+LAPs
                            if (mod(count,3)==0) write(un_result,*)
                            count = count+1
                            write(un_result,"(G24.15)",advance="NO") k 
                        end do
                    end do
                end do
        ...
        end subroutine save_Tecplot_Debug
